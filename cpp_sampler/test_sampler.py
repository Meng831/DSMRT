...
from cpp_sampler.online_sampler import build_query_tree
from common.util import query_name_dict, name_query_dict, tuple2list, list2tuple
...
def fill_query(query_tpl, query_args):
    assert isinstance(query_tpl, list)  #判断query_tpl是否是一个列表
    for i, e in enumerate(query_tpl):
        if isinstance(e, str):          #如果e是字符串，就把这个位置替换为查询列表
            query_tpl[i] = query_args[0]
            del query_args[0]
        else:                           #若是列表就再递归的执行替换操作
            fill_query(e, query_args)


class TestQuerySampler(object):   #执行查询采样操作
    def __init__(self, args, kg_small, kg_large, query_structures):     #构造函数，初始化类的实例
        for attr in ['neg_samples', 'max_num_answer', 'max_num_missing_answer', 'search_bandwidth', 'max_intermediate_answers']:
            assert getattr(args, attr) >= 0    #获取args上面的属性值，看他们是否都>=0
        ...
        for kg in [kg_small, kg_large]:
            list_qt = []            #用于存储查询树结构
            for qs in query_structures:                   #对于每个查询结构，调用 build_query_tree 函数创建查询树，并将其添加到 list_qt 列表中
    ...
    def collect_queries(self, query_type_idx, num_required, set_queries, dict_tp, dict_fp, dict_fn, dict_neg):
        ...
        query_str_template = tuple2list(query_str)     #将查询结构字符串query_str转换成列表的形式

        num_gen = 0
        for _ in tqdm(range(num_required)):        #循环将生成指定数量的查询，并将它们添加到相应的数据结构中
            while True:
                query_args, list_tp, list_fp, list_fn, list_neg = [], [], [], [], []
                assert self.query_sampler.fetch_query(query_args, list_tp, list_fp, list_fn, list_neg) == query_type_idx  #从查询采样器中获取查询参数和查询结果，并确保查询类型与 query_type_idx 相匹配
...
if __name__ == "__main__":
    ...
    if args.do_merge:
        ...
        #phases=['train','valid','test']       save_path = /home/jiang/data/knowledge_graphs/FB15k-237-betae/eval-miss-100-neg-1000
        for phase in phases:
            fnames = os.listdir(os.path.join(args.save_path, phase))
                                  #fnames=['train-queries.pkl', 'train-negative_samples.pkl', 'train-tp-answers.pkl', 'train-fn-answers.pkl', 'train-fp-answers.pkl']
            ...
            for fname in fnames:
                if not phase in fname:
                    continue
                cur_name = os.path.join(args.save_path, phase, fname)   #cur_name=/home/jiang/data/knowledge_graphs/FB15k-237-betae/eval-miss-100-neg-1000/train/rain-queries.pkl
                print('merging', fname)
