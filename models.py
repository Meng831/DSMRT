...
from common.util import name_query_dict, query_name_dict, thread_wrapped_func, log_metrics, tuple2filterlist, eval_tuple, flatten_query
...
class KGReasoning(nn.Module):
    def __init__(self, nentity, nrelation, ntype, hidden_dim, entity2type, relation2type, gamma,
        ...
        if self.geo == 'box':
            ...
            self.entity_embedding = nn.Parameter(torch.zeros(nentity, self.entity_dim)) # center for entities [14505, 800]
        elif self.geo == 'beta':
    ...
    def train_step(model, optimizer, train_iterator, args, step):
        ...
        for query_structure in batch_queries_dict:
            if args.cuda:
                batch_queries_dict[query_structure] = torch.stack(batch_queries_dict[query_structure]).long().cuda()
            else:
    ...
    @staticmethod
    def test_step(model, args, test_dataloader, query_name_dict, save_result=False, save_str="", save_empty=False):
        model.eval()
        ...
        with torch.no_grad():
            for negative_sample, queries, queries_unflatten, query_structures, easy_answers, hard_answers in tqdm(test_dataloader, disable=not args.print_on_screen):
            # for negative_sample, queries, queries_unflatten, query_structures in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                if args.cuda:
                    ...
                    if negative_sample is None:  # test against all entities
                        if all_embed is None:
                            entity_embedding = model.entity_embedding.data
                            all_embed = entity_embedding[torch.zeros(14505, dtype=torch.long), :]
                ...
                for idx, (i, query, query_structure,easy_answer, hard_answer) in enumerate(zip(argsort[:, 0], queries_unflatten, query_structures, easy_answers, hard_answers)):
